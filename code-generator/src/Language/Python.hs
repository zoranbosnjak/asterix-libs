...
instance Node A.Content where
    ...
    node ix = \case
        A.ContentRaw -> do
            out $ do
                fmt ("Content_" % int % "_Arg: TypeAlias = int") ix
                pyClass "Content" ix "ContentRaw" $ do
                    "pass"
        A.ContentTable values -> do
            let f (a, b) = sformat (int % ": " % stext) a (quote b)
            out $ do
                fmt ("Content_" % int % "_Arg: TypeAlias = int") ix
                pyClass "Content" ix "ContentTable" $ do
                    fmt ("cv_values = " % stext) (fmtList "{" "}" f values)
        A.ContentString st -> do
            out $ do
                fmt ("Content_" % int % "_Arg: TypeAlias = Union[int, str]") ix
                pyClass "Content" ix "ContentString" $ do
                    fmt ("cv_string_type = " % stext) $ case st of
        ...
        A.ContentInteger sig _cstr -> do
            out $ do
                fmt ("Content_" % int % "_Arg: TypeAlias = int") ix
                pyClass "Content" ix "ContentInteger" $ do
                    fmt ("cv_signedness = " % stext) $ case sig of
                        A.Signed   -> "Signed"
                        A.Unsigned -> "Unsigned"
        A.ContentQuantity sig lsb unit' _cstr -> do
            ...
            out $ do
                fmt ("Content_" % int %
                    "_Arg: TypeAlias = Union[int, float, Tuple[float, Literal[" %
                        stext % "]]]")
                    ix unit
                pyClass "Content" ix "ContentQuantity" $ do
                    fmt ("cv_signedness = " % stext) $ case sig of
                        A.Signed   -> "Signed"
                        A.Unsigned -> "Unsigned"
        ...
        A.ContentBds bt -> do
            out $ do
                fmt ("Content_" % int % "_Arg: TypeAlias = int") ix
                pyClass "Content" ix "ContentBds" $ do
                    case bt of
                        A.BdsWithAddress -> "cv_bds_type = BdsWithAddress"
                        A.BdsAt Nothing -> "cv_bds_type = (BdsAt, None)"
                        A.BdsAt (Just addr) -> fmt ("cv_bds_type = (BdsAt, " % int % ")")
                            (coerce addr :: Int)
...
instance Node (A.Rule A.Content) where
    ...
    node ix = \case
        A.ContextFree content -> do
            ref <- walk content
            out $ do
                fmt ("RuleContent_" % int % "_Arg: TypeAlias = Content_" % int
                     % "_Arg") ix ref
                pyClass "RuleContent" ix "RuleContentContextFree" $ do
                    fmt ("cv_content = Content_" % int) ref
                    ""
                    "@property"
        ...
        A.Dependent items dv cases -> do
            ...
            refs <- forM cases $ \(a, b) -> do
                ...
                pure (a, ref)
            out $ do
                fmt ("RuleContent_" % int % "_Arg = Union[") ix
                indent $ do
                    fmt ("Tuple[None, Content_" % int % "_Arg],") refDv
                    forM_ refs $ \(a, ref) -> do
                        let f = sformat ("Literal[" % int % "]")
                        fmt ("Tuple[" % stext % ", Content_" % int % "_Arg],")
                            (fmtList "Tuple[" "]" f a) ref
                "]"
                pyClass "RuleContent" ix "RuleContentDependent" $ do
                    let f1 (A.ItemPath path) = fmtList "[" "]" f2 path
                        f2 (A.ItemName name) = quote name
                    fmt ("cv_depends_on = " % stext)
                        (fmtList "[" "]" f1 items)
                    fmt ("cv_default_content = Content_" % int) refDv
                    "cv_cases = ["
...
instance Node Variation where
    ...
    node ix = \case
        A.Element o n rule -> do
            ...
            out $ do
                fmt ("Variation_" % int % "_Arg: TypeAlias = RuleContent_" % int
                     % "_Arg") ix ref
                pyClass "Variation" ix "Element" $ do
                    fmt ("cv_bit_offset8 = " % int) (coerce o :: Int)
                    fmt ("cv_bit_size = " % int) (coerce n :: Int)
                    fmt ("cv_rule = RuleContent_" % int) ref
                    ""
                    "@classmethod"
                    let arg = sformat ("arg : Variation_" % int % "_Arg") ix
                        rv = sformat("Variation_" % int) ix
                    pyFunc "create" ["cls", arg] (quote rv) $ do
                    ...
                    case mRef2 of
                        ...
                        Just ref2 -> do
                            ...
                            "# shortcut to content"
                            "@property"
                            pyFunc "content" ["self"] (sformat ("Content_" % int) ref2) $ do
        ...
        A.Group o lst -> do
            ...
            out $ do
                let ag = sformat ("Variation_" % int % "_Arg_Group") ix
                    f1 = \case
                        Nothing -> "int"
                        Just (name', ref) ->
                            let arg = sformat ("RuleVariation_" % int % "_Arg") ref
                                name = quote $ coerce name'
                                namedArg = sformat ("Tuple[Literal[" % stext % "], " % stext % "]")
                                    name arg
                            in sformat ("Union[" % stext % ", " % stext % "]") arg namedArg
                fmt (stext % " = " % stext) ag
                    (fmtList "Tuple[" "]" f1 refDict)
                fmt ("Variation_" % int % "_Arg: TypeAlias = Union[int, " % stext % "]") ix ag
                pyClass "Variation" ix "Group" $ do
                    fmt ("cv_bit_offset8 = " % int) (coerce o :: Int)
                    ...
                    fmt ("cv_items_dict = " % stext) (fmtList "{" "}" f3 (catMaybes refDict))
                    ""
                    case catMaybes refDict of
                        ...
                        [(name', ref)] -> do
                            ...
                            do
                                let arg3 = sformat
                                        ("arg : Variation_" % int % "_Arg") ix
                                    rv = sformat ("'Variation_" % int % "'") ix
                                "@classmethod"
                                pyFunc "create" ["cls", arg3] rv $ do
                                    "return cls._create(arg) # type: ignore"
                        refDict' -> do
                            forM_ refDict' $ \(name', ref) -> do
                                ...
                                pyFunc "spec" ["cls", arg] rv $ do
                                    "..."
                            "@classmethod"
                            let f4 (a, _) = sformat ("Literal[" % stext % "]") (quote $ coerce a)
                                arg4 = sformat("key : Union" % stext) (fmtList "[" "]" f4 refDict')
                            ...
                            ""
                            "@classmethod"
                            let arg2 = sformat ("arg : Variation_" % int % "_Arg") ix
                                rv = sformat ("'Variation_" % int % "'") ix
                            pyFunc "create" ["cls", arg2] rv $ do
                                "return cls._create(arg) # type: ignore"
            ...
            out $ do
                forM_ (zip [1::Int ..] groups) $ \(gi, group) -> do
                    let f = \case
                            ...
                            Just Nothing -> "int"
                            Just (Just (name, ref)) ->
                                let a1 = sformat ("RuleVariation_" % int % "_Arg") ref
                                    a2 = sformat ("Tuple[Literal[" % stext % "], " % stext % "]")
                                        (quote $ coerce name) a1
                                    a3 = sformat ("Union[" % stext % ", " % stext % "]") a1 a2
                                in a3
                        arg = fmtList "Union[int, Tuple[" "]]" f group
                    fmt ("Variation_" % int % "_Arg_Group_" % int
                         % ": TypeAlias = " % stext) ix gi arg
                fmt ("Variation_" % int % "_Arg: TypeAlias = Union[") ix
                indent $ forM_ (tail $ inits [1 .. length groups]) $ \gi -> do
                    let f = sformat ("Variation_" % int % "_Arg_Group_" % int) ix
                    fmt stext (fmtList "Tuple[" "]," f gi)
                "]"
                pyClass "Variation" ix "Extended" $ do
                    let f1 = \case
                            Nothing -> "None"
                            ...
                            Just (Just (name, ref)) -> do
                                let arg = sformat ("key : Literal[" % stext % "]")
                                    ...
                                    rv2 = case gi < 2 of
                                        ...
                                        False -> sformat ("Optional[RuleVariation_" % int % "]") ref
        ...
        A.Repetitive rt var -> do
            ...
            out $ do
                fmt ("Variation_" % int
                     % "_Arg: TypeAlias = List[Variation_" % int
                     % "_Arg]") ix ref
                pyClass "Variation" ix "Repetitive" $ do
                    case rt of
                        A.RepetitiveFx -> "cv_rep_bytes = None"
                        A.RepetitiveRegular n -> do
                            fmt ("cv_rep_bytes = " % int) (coerce n :: Int)
                    fmt ("cv_variation = Variation_" % int) ref
                    ""
                    let arg2 = sformat ("arg : Variation_" % int % "_Arg") ix
                        rv = sformat ("'Variation_" % int % "'") ix
                    "@classmethod"
        ...
        A.Explicit met -> do
            out $ do
                fmt ("Variation_" % int % "_Arg: TypeAlias = bytes") ix
                pyClass "Variation" ix "Explicit" $ do
                    fmt ("cv_explicit_type = " % stext) $ case met of
                        Nothing                  -> "None"
        ...
        A.Compound lst -> do
            ...
            out $ do
                fmt ("Variation_" % int % "_Arg = TypedDict('Variation_" % int
                     % "_Arg', {") ix ix
                indent $ forM_ refDict $ \(name, ref) -> do
                    fmt (stext % ": NonSpare_" % int % "_Arg,")
                        (quote $ coerce name) ref
                "}, total=False)"
                pyClass "Variation" ix "Compound" $ do
                    ...
                    case refDict of
                        ...
                        [(name', ref)] -> do
                            ...
                            pyFunc "get_item" ["self", arg] ("Optional[" <> rv <> "]") $ do
                                "return self._get_item(key) # type: ignore"
                            ""
                            let arg2 = sformat ("val : NonSpare_" % int % "_Arg")
                                    ref
                        ...
                        _ -> do
                            ...
                            forM_ refDict $ \(name, ref) -> do
                                ...
                                let key = sformat ("Literal[" % stext % "]")
                                        (quote $ coerce name)
                                    arg2 = sformat ("NonSpare_" % int % "_Arg") ref
...
instance Node (A.Rule Variation) where
    ...
    node ix = \case
        A.ContextFree var -> do
            ...
            out $ do
                fmt (stext % "_Arg: TypeAlias = Variation_" % int % "_Arg") rv ref
                pyClass "RuleVariation" ix "RuleVariationContextFree" $ do
                    fmt ("cv_variation = Variation_" % int) ref
                    ""
                    "@classmethod"
                    pyFunc "create" ["cls", "arg : " <> rv <> "_Arg"] (quote rv) $ do
                        "return cls._create(arg) # type: ignore"
                    ""
                    "@property"
        ...
        A.Dependent items dv cases -> do
            ...
            refs <- forM cases $ \(a, b) -> do
                ...
                pure (a, ref)
            out $ do
                fmt ("RuleVariation_" % int % "_Arg: TypeAlias = Union[") ix
                indent $ forM_ (refDv : fmap snd refs) $ \ref -> do
                    fmt ("Variation_" % int % "_Arg,") ref
                "]"
                pyClass "RuleVariation" ix "RuleVariationDependent" $ do
                    let f1 (A.ItemPath path) = fmtList "[" "]" f2 path
                        f2 (A.ItemName name) = quote name
                    fmt ("cv_depends_on = " % stext)
                        (fmtList "[" "]" f1 items)
                    fmt ("cv_default_variation = Variation_" % int) refDv
                    "cv_cases = ["
                    indent $ forM_ refs $ \(a, ref) -> do
                        let val = fmtList "[" "]" (sformat int) a
...
instance Node NonSpare where
    ...
    node ix (A.NonSpare name title rule _doc) = do
        ...
        out $ do
            fmt (stext % "_Arg: TypeAlias = RuleVariation_" %
                 int % "_Arg") clsName ref
            pyClass "NonSpare" ix "NonSpare" $ do
                fmt ("cv_name = " % stext) (quote $ coerce name)
                fmt ("cv_title = " % stext) (quote $ coerce title)
                fmt ("cv_rule = RuleVariation_" % int) ref
                ""
                "@classmethod"
                pyFunc "create" ["cls", "arg : " <> clsName <> "_Arg"] (quote clsName) $ do
                    "return cls._create(arg) # type: ignore"
                ""
                "@property"
                ...
                case mRef2 of
                    ...
                    Just ref2 -> do
                        ...
                        "@property"
                        pyFunc "variation" ["self"] (sformat ("Variation_" % int) ref2) $ do
...
instance Node Item where
    ...
    node ix = \case
        A.Spare o n -> do
            out $ do
                fmt ("Item_" % int % "_Arg: TypeAlias = int") ix
                pyClass "Item" ix "Spare" $ do
                    fmt ("cv_bit_offset8 = " % int) (coerce o :: Int)
                    fmt ("cv_bit_size = " % int) (coerce n :: Int)
        A.Item nsp -> do
            ref <- walk nsp
            ...
            out $ do
                fmt (stext % "_Arg: TypeAlias = NonSpare_" % int % "_Arg")
                    clsName ref
                pyClass "Item" ix "Item" $ do
                    fmt ("cv_non_spare = NonSpare_" % int) ref
                    ""
                    "@classmethod"
                    pyFunc "create" ["cls", "arg : " <> clsName <> "_Arg"] (quote clsName) $ do
                        "return cls._create(arg) # type: ignore"
    ...
    node ix = \case
        A.UapItem nsp -> do
            ...
            out $ pyClass "UapItem" ix "UapItem" $ do
                fmt ("cv_non_spare = NonSpare_" % int) ref
        A.UapItemSpare -> out $ pyClass "UapItem" ix "UapItemSpare" "pass"
        A.UapItemRFS -> out $ pyClass "UapItem" ix "UapItemRFS" "pass"
...
instance Node Record where
    ...
    node ix (Record items) = do
        ...
        out $ do
            fmt ("Record_" % int % "_Arg = TypedDict('Record_" % int % "_Arg', {")
                ix ix
            indent $ forM_ refDict $ \(name, ref) -> do
                fmt (stext % ": NonSpare_" % int % "_Arg,") name ref
            "}, total=False)"
            when rfs $ do
                fmt ("Record_" % int % "_Union = Union[") ix
                indent $ forM_ refDict $ \(name, ref) -> do
                    fmt ("Tuple[Literal[" % stext % "], NonSpare_" % int % "_Arg],")
                        name ref
                "]"
            pyClass "Record" ix "Record" $ do
                ...
                case refDict of
                    ...
                    _ -> do
                        ...
                        forM_ refDict $ \(name, ref) -> do
                            "@overload"
                            let arg = sformat ("Literal[" % stext % "]") name
                                arg3 = sformat ("NonSpare_" % int % "_Arg") ref
                                rv2 = sformat ("'Record_" % int % "'") ix
                ...
                do
                    ""
                    let arg_dict = sformat ("arg : Record_" % int % "_Arg") ix
                        arg_rfs = sformat ("rfs : Optional[List[Record_" % int
                            % "_Union]] = None") ix
                        args
...
instance Node Uap where
    ...
    node ix = \case
        A.Uap record -> do
            ref <- walk record
            out $ do
                fmt ("Uap_" % int % "_Arg: TypeAlias = Record_" % int) ix ref
                pyClass "Uap" ix "UapSingle" $ do
                    fmt ("cv_record = Record_" % int) ref
                    ""
                    "@classmethod"
        ...
        A.Uaps lst msel -> do
            ...
            out $ do
                ...
                fmt ("Uap_" % int % "_Arg: TypeAlias = Union" % stext) ix
                    (fmtList "[" "]" f refs)
                pyClass "Uap" ix "UapMultiple" $ do
                  do
                  ...
                  do
                    ""
                    "@classmethod"
                    let rv = sformat ("List[List[Uap_" % int % "_Arg]]") ix
                    pyFunc "parse_any_uap" ["cls", "bs : Bits"] rv $ do
...
instance Node Expansion where
    ...
    node ix (Expansion n lst) = do
        ...
        refDict <- forM (catMaybes lst) $ \case
            nsp@(A.NonSpare name _ _ _) -> (name,) <$> walk nsp
        out $ do
            fmt ("Expansion_" % int % "_Arg = TypedDict('Expansion_" % int
                    % "_Arg', {") ix ix
            indent $ forM_ refDict $ \(name, ref) -> do
                fmt (stext % ": NonSpare_" % int % "_Arg,")
                    (quote $ coerce name) ref
            "}, total=False)"
            pyClass "Expansion" ix "Expansion" $ do
                ...
                ""
                do
                    let arg2 = sformat ("arg : Expansion_" % int % "_Arg") ix
    ...
    node ix = \case
        AsterixBasic cat (A.Edition a b) uap -> do
            ...
            out $ pyClass "Asterix" ix "AstCat" $ do
                ...
                ""
                "@classmethod"
                let arg = sformat ("records : List[Uap_" % int % "_Arg]") ref
        ...
        AsterixExpansion cat (A.Edition a b) expan -> do
            ...
            out $ pyClass "Asterix" ix "AstRef" $ do
                fmt ("cv_category = " % int) (coerce cat :: Int)
                fmt ("cv_edition = (" % int % ", " % int % ")") a b
                fmt ("cv_expansion = Expansion_" % int) ref
